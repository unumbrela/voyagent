...
// 单城行程的 POI 应在目的地附近；超过此距离判为消歧错误（同名异地误配）。
// 曾用 600km——太松：苏州行程里的杭州(130km)/合肥(250km)同名误配全能通过，
// 针脚整段甩出目的地。100km 覆盖「主城 + 周边联游」（京都↔奈良、苏州↔无锡），
// 又能挡住邻省级别的漂移；省级目的地的远点交给高德 PlaceSearch 路径处理。
const MAX_KM = 100;
